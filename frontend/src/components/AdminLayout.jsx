...
function AdminLayout() {
  ...
  const menuItems = [
    ...
    { path: '/admin/assign', label: 'Assign Subjects', icon: FiLink },
    { path: '/admin/students', label: 'Students', icon: FiUser },
  ]
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-gray-50 via-purple-50 to-indigo-50">
      <div
        ...
        } bg-gradient-to-b from-purple-900 via-purple-800 to-indigo-900 text-white transition-all duration-300 flex flex-col shadow-2xl relative z-10`}
      >
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3 mb-2 rounded-xl transition-all duration-200 ${
                  isActive
                    ? 'bg-gradient-to-r from-purple-600 to-indigo-600 shadow-lg transform scale-105'
                    : 'hover:bg-white/10 hover:transform hover:translate-x-1'
                }`}
              >
                <Icon className={`w-5 h-5 ${isActive ? 'text-white' : 'text-purple-200'}`} />
                <span className={`${sidebarOpen ? 'block' : 'hidden'} font-medium ${isActive ? 'text-white' : 'text-purple-100'}`}>
                  {item.label}
                </span>
              </Link>
            )
